...
#### Unified Volumetric Sample Integration
Solver 3 handles samples not as infinitely thin planes, but as true Optical Volumes (slabs).
When a reverse-traced camera ray intersects the "near-plane" of the sample (the surface closest to the camera), the visual visualization trace is terminated because the ray has found the object it is imaging.
Mathematically, the solver proceeds to trace the path *internally* through the sample slab to its far-plane.
At the far-plane, it queries the local forward E&M field generated by Solver 2 (illumination).
It then integrates this illumination field forwards through the slab to the near-plane, applying Beer-Lambert absorption natively. Simultaneously, if the internal E&M field excites fluorescence, the corresponding emission is added to the volumetric integral (also subject to absorption).
This flawlessly unites Brightfield (absorption), Darkfield (scattering away from NA), and Fluorescence (emission from E&M excitation) into a single coherent physical Volume Render, naturally supporting self-shadowing and mixed-modality imaging.
...
### Core Principle: Time Is a Scene Graph Mutation
