...
type WebGpuFlagRegistry = typeof globalThis & {
    __MICROSCOPE_REVERSE_TRACE_ENABLE_WEBGPU__?: boolean;
};

export const REVERSE_TRACE_WEBGPU_INTERACTION_WGSL = `
struct SceneHeader {
    component_count: u32,
    matrix_stride: u32,
...
export interface ReverseTracerWebGpuPackedScene {
...
function canUseReverseTracerWebGpu(): boolean {
    return typeof navigator !== 'undefined'
        && typeof (navigator as NavigatorGpuLike).gpu?.requestAdapter === 'function';
}

function webGpuRequested(): boolean {
    return Boolean((globalThis as WebGpuFlagRegistry).__MICROSCOPE_REVERSE_TRACE_ENABLE_WEBGPU__);
}
...
export async function createReverseTracerWebGpuPackedScene(tracePacket: PackedTraceScene): Promise<ReverseTracerWebGpuPackedScene | null> {
    if (!canUseReverseTracerWebGpu()) return null;
    ...
    const buffers = [
        createStorageBuffer(device, 'reverseTrace-webgpu-header', header),
        createStorageBuffer(device, 'reverseTrace-webgpu-local-to-world', localToWorld),
        createStorageBuffer(device, 'reverseTrace-webgpu-interaction-kinds', interactionKinds),
        createStorageBuffer(device, 'reverseTrace-webgpu-surface-params', surfaceParams),
    ];
    const shader = device.createShaderModule({
        label: 'reverseTrace-packed-interactions',
        code: REVERSE_TRACE_WEBGPU_INTERACTION_WGSL,
    });
    ...
    return {
        ...
        bindGroup,
        dispatchPreflight() {
            const encoder = device.createCommandEncoder({ label: 'reverseTrace-packed-interactions-preflight' });
            const pass = encoder.beginComputePass({ label: 'reverseTrace-packed-interactions-preflight' });
...
class OptionalWebGpuReverseTracerBackend implements ReverseTracerKernelBackend {
    private readonly packedScene: Promise<ReverseTracerWebGpuPackedScene | null>;

    constructor(
        context: ReverseTracerKernelContext,
        private readonly fallback: ReverseTracerKernelBackend,
    ) {
        this.packedScene = createReverseTracerWebGpuPackedScene(context.tracePacket);
...
export function createOptionalWebGpuReverseTracerBackend(
    context: ReverseTracerKernelContext,
    fallback: ReverseTracerKernelBackend,
): ReverseTracerKernelBackend {
    if (!webGpuRequested() || !canUseReverseTracerWebGpu()) return fallback;
    return new OptionalWebGpuReverseTracerBackend(context, fallback);
}
