/**
 * Diagnostic test: Runs Reverse tracer on the Epi-Fluorescence preset and logs
 * the backward ray trace interaction chain. This tests why Reverse tracer
...
function runReverseTracerDebug(presetName: string, createScene: () => any[]) {
    test(`${presetName}: Reverse tracer backward trace diagnostics`, () => {
        ...
        // Step 1: forward tracer — trace forward rays
        ...
        const { paths, beamSegments: beamSegs } = forwardTracer.traceWithBeamSegments(sourceRays);
        console.log(`  forward tracer paths: ${paths.length}`);
        console.log(`  Beam field beamSeg branches: ${beamSegs.length}`);

        // Step 3: Reverse tracer — backward trace from camera
        ...
        const result = reverseTrace.traceBackward(backwardRay, sample);
...
describe("Reverse tracer Backward Trace Diagnostics", () => {
    runReverseTracerDebug("Brightfield", createBrightfieldScene);
    runReverseTracerDebug("Epi-Fluorescence", createEpiFluorescenceScene);
});
