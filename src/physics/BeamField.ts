...
export class BeamField {
    /**
     * Compatibility wrapper: beamlet construction now lives in forward tracer.
     */
    propagate(
        allPaths: Ray[][],
        components: OpticalComponent[]
    ): GaussianBeamSegment[][] {
        return new ForwardTracer(components).buildBeamSegments(allPaths);
    }
    ...
    static queryIntensityMultiBeam(
        ...
        if (branches.length === 1) {
            const result = BeamField.queryIntensity(px, py, pz, branches[0].segments);
        ...
        for (const branch of branches) {
            const result = BeamField.queryIntensity(px, py, pz, branch.segments);
