/**
 * reverse-tracer Web Worker — runs the reverse (backward) ray trace off the main
 ...
 *                    reversePathCount, forwardRayCount, beamFieldEnabled,
 *                    emissionWavelengthM }
...
import { ReverseTracerKernel } from './reverseTraceKernel';
...
import { ensureReverseTracerWasmLoaded } from './reverseTraceWasmLoader';
import {
...
async function runPMTScan(
    ...
    for (const pmt of pmts) {
        ...
        for (let i = 0; i < req.reversePathCount; i++) {
            ...
            const r = reverseTrace.traceBackward(backwardRay, sample);
            ...
            for (let yIdx = 0; yIdx < plan.resY; yIdx++) {
                ...
                for (let xIdx = 0; xIdx < plan.resX; xIdx++) {
                    ...
                    try {
                        ...
                        const beamSegs = new ForwardTracer(components).traceWithBeamSegments(sourceRays).beamSegments;
                        ...
                        const reverseTrace = new ReverseTracerKernel(createTraceSceneSnapshot(components), { branches: beamSegs });
                        const { radiance, excitation, bestPath } = reverseTrace.renderPMTPixel(pmtSnapshot);
...
async function runJob(req: RenderRequest) {
    ...
    try {
        ...
        // reverse-tracer needs beam segments (even a coarse estimate) to compute
        ...
        let beamSegs: ReturnType<typeof forwardTracer.traceWithBeamSegments>['beamSegments'] = [];
        try {
            beamSegs = forwardTracer.traceWithBeamSegments(sourceRays).beamSegments;
        } catch {
            beamSegs = [];
        }
        ...
        // Make sure the Rust kernel is available before instantiating ReverseTracer
        // (its backend snapshot happens in the constructor).  If the kernel
        // fails to load we fall through to the JS backend, which ReverseTracer
        // handles automatically.
        await ensureReverseTracerWasmLoaded();
        ...
        for (let round = 0; ; round++) {
            ...
            for (let ci = 0; ci < cameras.length; ci++) {
                ...
                const gen = reverseTrace.renderGenerator(cam, req.previewMode ? 8 : 32, req.workerId, req.workerCount, activeMask);
            ...
            if (firstRoundDone && !pmtDone) {
                await runPMTScan(req, pmts, sample, reverseTrace, (r) => { raysTraced += r; });
                pmtDone = true;
            }
