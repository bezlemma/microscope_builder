...
import { Vector3, Vector2, LatheGeometry, DoubleSide } from 'three';
...
export class SphericalLens extends OpticalComponent {
    ...
    // ─── Dispersion / Material ────────────────────────────────────────
    /** Material definition — when set, IoR is computed from wavelength. */
    ...
    /**
     * Get effective IoR for a given wavelength.
     * If a material with dispersion is set, computes n(λ).
     * Otherwise falls back to the constant `ior` field.
     */
    getEffectiveIor(wavelengthMeters: number): number {
        if (this.material && this.material.formula !== DispersionFormula.NONE) {
            ...
            return calculateRefractiveIndex(this.material, wavelengthNm);
        }
        return this.ior;
    }

    /**
     * Set a named material for dispersion.
     * Also updates the constant `ior` to the material's reference value
     * so that non-wavelength-aware code still works.
     */
    setMaterial(material: MaterialDef): void {
        this.material = material;
        this.ior = material.n; // Keep constant IoR in sync as reference
        this.invalidateMesh();
    }

    /** Clear material and revert to constant IoR mode. */
    clearMaterial(): void {
        this.material = null;
    }
     ...
     * Returns [A, B, C, D].
     */
    getABCD(_rayDirection?: import('three').Vector3, wavelengthSI?: number): [number, number, number, number] {
        const { R1, R2 } = this.getRadii();
        ...
        >
            <mesh rotation={[0, 0, Math.PI / 2]}>
                ...
                <meshPhysicalMaterial
                    ...
                    side={DoubleSide}
                    depthWrite={false}
                    emissive={isSelected ? "#007fff" : "#000000"}
